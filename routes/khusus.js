...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        const uploadDir = path.join(__dirname, '../public/uploads/khusus');
...
function formatFilesData(files) {
    ...
    if (files.length === 1) {
        return {
            ...
            filePaths: '/uploads/khusus/' + files[0].filename,
            fileSizes: files[0].size
        };
    } else {
        const fileNames = files.map(f => f.originalname);
        const filePaths = files.map(f => '/uploads/khusus/' + f.filename);
...
// 📌 Daftar Isu Khusus dengan Enhanced Filtering
router.get('/', (req, res) => {
    ...
    connection.query(countQuery, countParams, (err, countResult) => {
        if (err) {
            console.error(err);
            return res.render('LaporanPimpinan/khusus/index', {
                title: 'Isu Khusus',
                currentPage: 'isu-khusus',
                data: [],
                pagination: null,
        ...
        connection.query(query, params, (err, rows) => {
            ...
            processRowsWithOPDNames(rows, (processedRows) => {
                ...
                const pagination = {
                    ...
                    prevUrl: page > 1 ? buildUrlWithParams('/isu-khusus', {
                        ...filterParams,
                        page: page > 2 ? page - 1 : null
                    }, ['page']) : null,
                    nextUrl: page < totalPages ? buildUrlWithParams('/isu-khusus', {
                        ...filterParams,
                        page: page + 1
                    }) : null,
                    firstUrl: buildUrlWithParams('/isu-khusus', filterParams, ['page']),
                    lastUrl: buildUrlWithParams('/isu-khusus', {
                        ...filterParams,
                        page: totalPages
                    })
                };

                res.render('LaporanPimpinan/khusus/index', {
                    title: 'Isu Khusus',
                    currentPage: 'isu-khusus',
                    data: processedRows,
...
router.get('/create', (req, res) => {
    ...
    connection.query('SELECT id, nama_opd FROM opd WHERE is_deleted = 0 ORDER BY nama_opd ASC', (err, opdData) => {
        ...
        res.render('LaporanPimpinan/khusus/create', {
            title: 'Tambah Isu Khusus',
            currentPage: 'isu-khusus',
            judul: '',
            tanggal_laporan: '',
            isi_laporan: '',
...
router.post('/store', upload.array('files'), (req, res) => {
    ...
    connection.query('INSERT INTO laporan_pimpinan SET ?', formData, (err) => {
        ...
        res.redirect('/isu-khusus');
    });
});

// 📌 Form edit dengan OPD data 
router.get('/edit/:id', (req, res) => {
    let id = req.params.id;

    // Get data laporan dan OPD
    const queries = [
        'SELECT * FROM laporan_pimpinan WHERE id = ? AND jenis_laporan = 2 AND is_deleted = 0',
    ...
    connection.query(queries[0], [id], (err, rows) => {
        ...
        connection.query(queries[1], (err, opdData) => {
            ...
            res.render('LaporanPimpinan/khusus/edit', {
                title: 'Edit Isu Khusus',
                currentPage: 'isu-khusus',
                id: data.id,
                judul: data.judul,
    ...
    connection.query('SELECT file_name, file_path, file_size FROM laporan_pimpinan WHERE id = ?', [id], (err, result) => {
        ...
        if (req.files && req.files.length > 0) {
            const newFiles = req.files.map(file => ({
                ...
                path: '/uploads/khusus/' + file.filename,
                size: file.size
            }));
        ...
        connection.query(
            'UPDATE laporan_pimpinan SET ? WHERE id = ? AND jenis_laporan = 2',
            [formData, id],
            (err) => {
                if (err) {
                    console.error(err);
...
router.get('/download/:id/:fileIndex', (req, res) => {
    ...
    connection.query(
        'SELECT file_name, file_path FROM laporan_pimpinan WHERE id = ? AND jenis_laporan = 2 AND is_deleted = 0',
        [id],
        (err, rows) => {
            if (err) {
                console.error(err);
...
router.get('/download-all/:id', (req, res) => {
    ...
    connection.query(
        'SELECT judul, file_name, file_path FROM laporan_pimpinan WHERE id = ? AND jenis_laporan = 2 AND is_deleted = 0',
        [id],
        (err, rows) => {
            if (err) {
                console.error(err);
...
router.get('/delete/:id', (req, res) => {
    let id = req.params.id;
    connection.query(
        'UPDATE laporan_pimpinan SET is_deleted = 1, updated_by = ?, updated_at = NOW() WHERE id = ? AND jenis_laporan = 2',
        ['admin', id],
        (err) => {
            if (err) {
                console.error(err);
