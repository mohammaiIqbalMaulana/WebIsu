...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        const uploadDir = path.join(__dirname, '../public/uploads/viralitas');
...
function formatFilesData(files) {
    ...
    if (files.length === 1) {
        return {
            ...
            filePaths: '/uploads/viralitas/' + files[0].filename,
            fileSizes: files[0].size
        };
    } else {
        const fileNames = files.map(f => f.originalname);
        const filePaths = files.map(f => '/uploads/viralitas/' + f.filename);
...
// 📌 Daftar Isu Viralitas dengan Enhanced Filtering
router.get('/', (req, res) => {
    ...
    connection.query(countQuery, countParams, (err, countResult) => {
        if (err) {
            console.error(err);
            return res.render('LaporanPimpinan/viralitas/index', {
                title: 'Isu Viralitas',
                currentPage: 'viralitas',
                data: [],
                pagination: null,
        ...
        connection.query(query, params, (err, rows) => {
            ...
            processRowsWithOPDNames(rows, (processedRows) => {
                ...
                const pagination = {
                    ...
                    prevUrl: page > 1 ? buildUrlWithParams('/viralitas', {
                        ...filterParams,
                        page: page > 2 ? page - 1 : null
                    }, ['page']) : null,
                    nextUrl: page < totalPages ? buildUrlWithParams('/viralitas', {
                        ...filterParams,
                        page: page + 1
                    }) : null,
                    firstUrl: buildUrlWithParams('/viralitas', filterParams, ['page']),
                    lastUrl: buildUrlWithParams('/viralitas', {
                        ...filterParams,
                        page: totalPages
                    })
                };

                res.render('LaporanPimpinan/viralitas/index', {
                    title: 'Isu Viralitas',
                    currentPage: 'viralitas',
                    data: processedRows,
...
router.get('/create', (req, res) => {
    ...
    connection.query('SELECT id, nama_opd FROM opd WHERE is_deleted = 0 ORDER BY nama_opd ASC', (err, opdData) => {
        ...
        res.render('LaporanPimpinan/viralitas/create', {
            title: 'Tambah Isu Viralitas',
            currentPage: 'viralitas',
            judul: '',
            tanggal_laporan: '',
            isi_laporan: '',
...
router.get('/edit/:id', (req, res) => {
    ...
    const queries = [
        'SELECT * FROM laporan_pimpinan WHERE id = ? AND jenis_laporan = 3 AND is_deleted = 0',
    ...
    connection.query(queries[0], [id], (err, rows) => {
        ...
        connection.query(queries[1], (err, opdData) => {
            ...
            res.render('LaporanPimpinan/viralitas/edit', {
                title: 'Edit Isu Viralitas',
                currentPage: 'viralitas',
                id: data.id,
                judul: data.judul,
    ...
    connection.query('SELECT file_name, file_path, file_size FROM laporan_pimpinan WHERE id = ?', [id], (err, result) => {
        ...
        if (req.files && req.files.length > 0) {
            const newFiles = req.files.map(file => ({
                ...
                path: '/uploads/viralitas/' + file.filename,
                size: file.size
            }));
        ...
        connection.query(
            'UPDATE laporan_pimpinan SET ? WHERE id = ? AND jenis_laporan = 3',
            [formData, id],
            (err) => {
                if (err) {
                    console.error(err);
...
router.get('/download/:id/:fileIndex', (req, res) => {
    ...
    connection.query(
        'SELECT file_name, file_path FROM laporan_pimpinan WHERE id = ? AND jenis_laporan = 3 AND is_deleted = 0',
        [id],
        (err, rows) => {
            if (err) {
                console.error(err);
...
router.get('/download-all/:id', (req, res) => {
    ...
    connection.query(
        'SELECT judul, file_name, file_path FROM laporan_pimpinan WHERE id = ? AND jenis_laporan = 3 AND is_deleted = 0',
        [id],
        (err, rows) => {
            if (err) {
                console.error(err);
...
router.get('/delete/:id', (req, res) => {
    let id = req.params.id;
    connection.query(
        'UPDATE laporan_pimpinan SET is_deleted = 1, updated_by = ?, updated_at = NOW() WHERE id = ? AND jenis_laporan = 3',
        ['admin', id],
        (err) => {
            if (err) {
                console.error(err);
